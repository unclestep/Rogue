...
All three pools are committed (3000 maps total, ~5 MB). Regenerate them only if the Go BSP generator
changes (rules, room placement, corridor carving) so the training distribution stays in sync with
the runtime game.
